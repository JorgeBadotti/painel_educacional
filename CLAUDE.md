...
**Authentication gates the whole app.** `App.tsx` wraps everything in `AuthProvider` (`src/context/AuthContext.tsx`) and renders `LoginScreen` until a real account is logged in — `CockpitProvider`/`MainCockpitContent` only mount after that. This app is the **shared identity provider** for the sibling `smart-teacher` repo: accounts live in a SQLite `users` table (`server/db.ts`) owned only by this server, and `POST /api/auth/login|register` return a JWT (via `server/auth.ts`) containing the full profile (id/name/email/role/municipality) — not just a user id — so any server holding the same `JWT_SECRET` can verify a token and know who's calling without a DB lookup or network round-trip back here. `smart-teacher`'s frontend calls this server's `/api/auth/*` directly (cross-origin; see `ALLOWED_ORIGINS` in `.env.example`), and its backend verifies the same tokens statelessly. The token is stored client-side (`localStorage`, key `educacao_auth_token`) and attached via `authFetch()`/`Authorization: Bearer` — not an httpOnly cookie — specifically so auth works across the two apps' different origins/ports. `requireAuth` (server/auth.ts) protects `POST /api/radar-analysis`; add it to any new backend endpoint that shouldn't be callable anonymously.

**Single Context, no external state library.** Almost all app state (KPIs, schools, alerts, highlights, action plans, municipality config, active tab, selected filters, TV mode, live-streaming toggle) lives in one provider: `src/context/CockpitContext.tsx`, consumed via the `useCockpit()` hook. It takes the logged-in user's profile as an `initialUserProfile` prop from `App.tsx` (not from its own login form). `App.tsx`'s `MainCockpitContent` is a thin tab switcher driven by `activeTab` (`ActiveTab` union) that renders one top-level view component per tab, plus a special fullscreen `isTvMode` branch (`TvPresentationMode`) that bypasses the normal layout entirely.
